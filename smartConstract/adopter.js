...
var AdopterContract = function() {
	...
	//设置等级属性
	 LocalContractStorage.defineProperty(this,"freeType");
	 LocalContractStorage.defineProperty(this,"oneNasType");
	 LocalContractStorage.defineProperty(this,"tenNasType");
	 LocalContractStorage.defineProperty(this,"hundresType");
	 //喂养时间
	 LocalContractStorage.defineProperty(this,"feedTime");
};
...
AdopterContract.prototype = {
	...
	createNewUser : function(phone,nickName,animalName,invitedCode) {
		...
		var freeType = this.freeType;
		var oneNasType = this.oneNasType;
		var tenNasType = this.tenNasType;
		var hundresType = this.hundresType;
	...
	feed : function() {
		...
		if ((Date.now() - object.lastFeedTime) <= 3600 * this.feedTime) {
	...
	//修改等级属性
	setLevelType : function(freeType,oneNasType,tenNasType,hundresType) {
		//判断是否为管理员
		...
		this.oneNasType = oneNasType;
		this.tenNasType = tenNasType;
		this.hundresType = hundresType;
	},

	//修改喂养时间
	setFeedTime : function(newFeedTime) {
		//判断是否为管理员
		if (Blockchain.transaction.from != this.adminAddress) {
			throw new Error("Permission denied");
		}
		this.feedTime = newFeedTime;
	},
	...
	withDraw : function (value) {
		...
		if (this.moneyPool > 0) {	//如果资金池中存在用户资金，则管理员无法提现
			throw new Error("Could not take out money before user take out their's");
		}
